...
A proposition is a type, a proof is a term of that type, and `h : x = y` in the
Infoview says what `n : ℕ` says. The four tactics of the day — `exact`,
`apply`, `intro`, `rfl` — are four operations on that relation.

Here's the session if you want to start it on the Lean server (a little slow,
but requires no installation work): *link to be generated with
`scripts/weblink.sh` once this folder is pushed.*

## Notes

- **§1–§5 carry `NOW TRY THIS` blocks**, as in session 1: at your own keyboard,
  ten seconds each, written so that following them produces what the text says.
- **`(2 : ℝ)` is not a cast.** It says to build the numeral with `ℝ` as the
  ...
  converting a natural one. §1 explains what elaboration is; the distinction
  matters in §5.
- **The `rfl` failure in §4** prints `rfl`'s type next to the goal's type.
  Uncomment it, read it, put the dashes back.
